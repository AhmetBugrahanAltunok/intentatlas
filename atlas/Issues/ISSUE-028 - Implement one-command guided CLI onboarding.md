...
- [ ] Confirm a clean synchronized starting revision and record it in EVD-030 before code changes.

## Work package 14A - guided contract and safe session engine

- [ ] Add an isolated onboarding/session module with injected terminal I/O and centralized EN/TR
      human-copy catalogs; add no runtime dependency.
- [ ] Add explicit `intentatlas guide [PATH]` and retain every existing parser route and output.
- [ ] Resolve one explicit or nearest Git root without broad filesystem discovery, history, shell
      evaluation, unsafe symlink/junction traversal, or Private access.
- [ ] Select conflict/worktree/staged/HEAD/unborn scopes deterministically from bounded Git metadata
      and expose alternate explicit scope selection without guessing a base branch.
- [ ] Add line-oriented progress, cancellation, terminal sanitization, path normalization, narrow-
      terminal, encoding, `NO_COLOR`, `TERM=dumb`, EOF, and error recovery contracts.
- [ ] Prove that no default guided operation writes, makes an external request, or executes project
      code, tests, hooks, tools, plugins, or indexers.

## Work package 14B - zero-argument progressive report

- [ ] Route empty argv to the guide only for interactive stdin/stdout; preserve non-TTY/redirected/
      CI argparse stderr, exit 2, no-scan, no-read, and no-wait behavior.
- [ ] Show canonical root, safe default scope, and trust statement before at most one Enter.
- [ ] Build one production graph/ChangeReport context and render the concise guided answer without
      duplicate scoring, resolving, freshness, fallback, or omission logic.
- [ ] Expose exact revision/scope, state/freshness, threshold, counts, score/confidence/reasons,
      omissions, strategy, advisory, and `tests executed: 0` with plain-language mappings.
- [ ] Add progressive evidence paths, omitted-candidate reasons, alternate scopes, reproducible
      explicit text/JSON commands, language switching, and immediate safe exit.
- [ ] Lock semantic parity for targeted, targeted-plus-full-suite, full-suite-fallback,
...
- [ ] Open the already-created snapshot only after explicit selection, on IPv4 loopback and an
      ephemeral port, with existing Host/header/escaping/accessibility/bounded-query controls.
- [ ] Keep persistent `init`, `scan`, `open`, test execution, shell evaluation, and automatic browser
      behavior out of the guide; provide commands only after the first answer.
- [ ] Update README/README.tr, quick start, trust-first preview, observation guide, architecture,
      compatibility policy, changelog, and release/package inclusion checks.
- [ ] Add focused transcripts, real PTY/non-TTY checks where available, clean-installed-wheel E2E,
...
- [ ] Add exact Code and Test links only after implementation artifacts exist.
