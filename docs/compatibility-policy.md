...
| Guided CLI human interaction | stable | empty argv TTY gate, `guide [PATH]`, conservative scope order, one-Enter confirmation, no-write boundary, and Change Report trust meanings | Non-TTY empty argv retains argparse stderr/exit 2. Additive prompts are allowed, but existing scope, omission, fallback, advisory, and explicit-browser meanings do not silently weaken. |
