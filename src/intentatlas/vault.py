...
class ProjectVault:
    ...
    def _note_locations(self, graph: AtlasGraph) -> dict[str, Path]:
        ...
        for node in sorted(graph.nodes.values(), key=lambda item: item.id):
            ...
            if key in used and used[key] != node.id:
                ...
                suffix = node.id.encode("utf-8").hex()[-8:]
                filename = safe_filename(f"{stem} — {suffix}")
            used[f"{area}/{filename}".casefold()] = node.id
    ...
    def _render_node(
        ...
        frontmatter.extend(["---", GENERATED_MARKER, "", f"# {note_title(node)}", ""])
        ...
        if metadata:
            ...
            for key, value in sorted(metadata.items()):
                ...
                frontmatter.append(f"- **{key.replace('_', ' ').title()}:** `{rendered}`")
            frontmatter.append("")
    ...
    def _render_edges(
        ...
        for edge in sorted(edges, key=lambda item: (item.relation, item.source, item.target)):
            ...
            lines.append(
                f"- `{edge.relation}` {arrow} [[{link}|{other.label}]] "
                f"_(evidence: {edge.evidence})_"
            )
        lines.append("")
        return lines
    ...
    def _render_dashboard(graph: AtlasGraph, locations: dict[str, Path]) -> str:
        ...
        for node in connected:
            ...
            lines.append(f"- [[{link}|{node.label}]] — {graph.degree(node.id)} relationships")
        lines.extend(
            [
                "",
                "## Workflow",
